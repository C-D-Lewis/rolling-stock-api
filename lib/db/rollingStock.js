...
const { insert, findOne, deleteOne } = require('./index');
...
/**
 * Find a document from the DB using a filter.
 *
 * @param {Object} filter - Request payload.
 * @returns {Object} Resulting resource.
 */
exports.find = async (filter) => {
  const res = await findOne(COLLECTION_NAME, filter);
  return res ? omit(res, ['_id']) : null;
};
